...
The `import_batch_id` column allows tracing which users were created by which import operation. It is nullable (existing users will have NULL).
...
- Error reports are scoped to the uploading user — other users cannot access them.
...
- Health check unaffected — import is synchronous and does not add background processes.

## Testing Strategy

- Unit tests: CSV parser (valid/invalid files), structural validator (all edge cases), business validator (duplicates, types), import executor (success, rollback), error report (format).
...
- Database migration (`add-import-batch-id`) must run before deployment.
- Migration is backwards-compatible (nullable column, no breaking changes).
- No feature flag needed — endpoint is new and does not affect existing functionality.
- Error report cleanup cron must be configured in deployment (24h TTL for `/tmp/import-reports/`).
